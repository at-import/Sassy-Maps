...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    concat: {
      rb: {
        ...
        files: {
          '.compass/lib/sassy-maps.rb': ['.compass/.template/extension.rb']
        }
      },
      gemspec: {
        options: {
        ...
        files: {
          '.compass/sassy-maps.gemspec': ['.compass/.template/extension.gemspec']
        }
      }
    },
    ...
    exec: {
      ...
      ext: {
        cmd: 'cd .compass && bundle exec gem build sassy-maps.gemspec && mv sassy-maps-' + userConfig.client.version + '.gem ../sassy-maps-' + userConfig.client.version + '.gem && cd ..'
      },
      install: {
        cmd: 'gem install sassy-maps-' + userConfig.client.version + '.gem && rm sassy-maps-' + userConfig.client.version + '.gem'
      },
      bundle: {
        cmd: function(path) {
          if (path === '.') {
  ...
  grunt.registerTask('extension', 'Build your Compass Extension', function() {
    ...
    gemfile += "\ngem 'Sassy Maps', '~>" + grunt.userConfig.client.version + "'";
